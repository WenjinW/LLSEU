...
def get(path, seed=0, pc_valid=0.10):
    ...
    for i in range(5):
        ...
        for s in ['train', 'test']:
            data[i][s] = {'x':[],'y':[]}
            data[i][s]['x'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_cifar'),'data'+str(ids[i]+5)+s+'x.bin'))
            data[i][s]['y'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_cifar'),'data'+str(ids[i]+5)+s+'y.bin'))
