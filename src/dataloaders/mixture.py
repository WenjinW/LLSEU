...
from dataloaders.my_dataset import MyEMNIST

########################################################################################################################


def get(path='../dat/', seed=0, fixed_order=False, pc_valid=0.15):
    data = {}
    taskcla = []
    size = [3, 32, 32]
    ...
    if not os.path.isdir(data_path+'binary_mixture/'):
        os.makedirs(data_path+'binary_mixture')
        # Pre-load
        for n, idx in enumerate(idata):
            if idx == 0:
                ...
                mean = (0.1722,)
                std = (0.3309,)
                dat['train'] = MyEMNIST(data_path, split='letters', train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(), transforms.Normalize(mean, std)]))
                dat['test'] = MyEMNIST(data_path, split='letters', train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(), transforms.Normalize(mean, std)]))
                data[n] = {}
                data[n]['name'] = 'emnist_letters'
                data[n]['ncla'] = 28
                for s in ['train','test']:
                ...
                std = (0.3318,)
                dat = {}
                dat['train'] = datasets.FashionMNIST(data_path+'fashion_mnist', train=True, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                dat['test'] = datasets.FashionMNIST(data_path+'fashion_mnist', train=False, download=True, transform=transforms.Compose([
                    transforms.Pad(padding=2, fill=0), transforms.ToTensor(),transforms.Normalize(mean, std)]))
                data[n] = {}
                ...
                # KMNIST
                mean = (0.1918,)  # Mean and std including the padding
                std = (0.3483,)
                dat = {}
                dat['train'] = datasets.KMNIST(data_path, train=True, download=True,
                                                     transform=transforms.Compose([
                                                         transforms.Pad(padding=2, fill=0), transforms.ToTensor(),
                                                         transforms.Normalize(mean, std)]))
                dat['test'] = datasets.KMNIST(data_path, train=False, download=True,
                                                    transform=transforms.Compose([
                                                        transforms.Pad(padding=2, fill=0), transforms.ToTensor(),
                                                        transforms.Normalize(mean, std)]))
                data[n] = {}
            ...
            for s in ['train', 'test']:
                ...
                torch.save(data[n][s]['x'], os.path.join(os.path.expanduser(data_path+'binary_mixture'),
                                                         'data'+str(idx)+s+'x.bin'))
                torch.save(data[n][s]['y'], os.path.join(os.path.expanduser(data_path+'binary_mixture'),
        ...
        for n, idx in enumerate(idata):
            ...
            for s in ['train', 'test']:
                data[n][s] = {'x': [],'y': []}
                data[n][s]['x'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_mixture'),
                                                          'data'+str(idx)+s+'x.bin'))
                data[n][s]['y'] = torch.load(os.path.join(os.path.expanduser(data_path+'binary_mixture'),
                                                          'data'+str(idx)+s+'y.bin'))
