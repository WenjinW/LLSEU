...
class ResNet(nn.Module):
    ...
    def __init__(self, input_size, task_class, block, layers, norm_layer=nn.BatchNorm2d):
        ...
        # size = 16 * 16 * 64
        self.layer2, arch_2 = self._make_layer(block, 128, layers[1], stride=2)
        # size = 8 * 8 * 128
        self.layer3, arch_3 = self._make_layer(block, 256, layers[2], stride=2)
        # size = 4 * 4 * 256
        self.layer4, arch_4 = self._make_layer(block, 512, layers[3], stride=2)
        # size = 2 * 2 * 512
