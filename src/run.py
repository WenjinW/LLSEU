...
parser.add_argument('--mode', default='search', type=str, required=False, choices=['train', 'search'],
                    help='(default=%(default)s)')
...
parser.add_argument('--model', default='alexnet', type=str, required=False, choices=['alexnet', 'resnet', 'mlp', 'auto'],
                    help='(default=%(default)s)')
...
parser.add_argument('--c_batch', default=512, type=int, required=False, help='(default=%(default)d)')
...
parser.add_argument('--o_batch', default=512, type=int, required=False, help='(default=%(default)d)')
...
print('Load data...')
if args.location == 'polyaxon':
    data, taskcla, inputsize = dataloader.get(path='/plx-data/xxx/', seed=args.seed)
else:
...
if args.location == 'polyaxon':
    ...
    with open("../res/" + exp_name + '.json', 'w') as f:
        json.dump(result, f)
